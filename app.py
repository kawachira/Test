...
def ai_hybrid_analysis(price, ema20, ema50, ema200, rsi, macd_val, macd_sig, adx, bb_up, bb_low, vol_status, mtf_trend, atr_val, mtf_ema200_val):
    ...
    # 🌟 Situation Insight (ตัวที่หายไป กู้คืนแล้ว!)
    situation_insight = ""
    ...
    if score >= 6:
        status, color, action = "🚀 Super Nova", "green", "Aggressive Buy"
        context = "ทุก Timeframe เป็นขาขึ้น วอลุ่มซื้อถล่มทลาย"
        holder_advice = "🎉 ถือต่อ (Let Profit Run) ใช้ Trailing Stop เกาะเทรนด์ไป"
    elif score >= 4:
        status, color, action = "🐂 Strong Bullish", "green", "Buy / Hold"
        context = "เทรนด์ขาขึ้นชัดเจน โมเมนตัมบวก"
        holder_advice = "🥳 ถือต่อได้สบายใจ ถ้ามีย่อตัวใกล้ EMA 20 ถือเป็นโอกาสเก็บเพิ่ม"
    elif score >= 2:
        status, color, action = "📈 Moderate Bullish", "green", "Buy on Dip"
        context = "ภาพรวมขาขึ้น แต่ระยะสั้นมีการพักตัว"
        holder_advice = "🙂 ถือต่อได้ แต่ถ้าหลุด EMA 20 ให้แบ่งขายทำกำไรบางส่วน"
    elif score >= -1:
        status, color, action = "⚖️ Neutral", "yellow", "Wait & See"
        context = "แรงซื้อขายสูสี รอเลือกทาง"
        holder_advice = "🤔 ถ้าทุนต่ำถือรอได้ ถ้าทุนสูงให้ตั้ง Stop Loss ที่กรอบล่าง"
    elif score >= -3:
        status, color, action = "☁️ Weak Warning", "orange", "Defensive"
        context = "โมเมนตัมแผ่วลง ระวังฐานแตก"
        holder_advice = "🦅 อย่าเพิ่งรับมีด! รอให้ราคากลับมายืนเหนือ EMA 20 ให้ได้ก่อน"
    else:
        status, color, action = "🐻 Strong Bearish", "red", "Avoid / Cut Loss"
        context = "หลุดแนวรับสำคัญ เทรนด์เปลี่ยนเป็นขาลง"
        holder_advice = "🥶 Cut Loss Now! อย่าเสียดาย ห้ามถัวเฉลี่ยขาลงเด็ดขาด"

    return {
        "status": status, "color": color, "action": action,
        "context": context, "bulls": bullish_factors, "bears": bearish_factors,
...
if submitted and symbol_input:
    with st.spinner("AI กำลังทำงาน..."):
        ...
        if df is not None and len(df) > 50:
            ...
            # --- 🔥 BANKER'S COST LOGIC (กู้คืนมาแล้ว!) ---
            banker_price = np.nan
            if len(df) >= 90:
                last_90 = df.tail(90)
...
if st.session_state['analyzed_data']:
    ...
    with col_head:
        ...
        if info['preMarketPrice']: st.markdown(f'<div style="margin-top:5px;">☀️ Pre: <b>{info["preMarketPrice"]:.2f}</b> {make_pill(info["preMarketChange"], info["preMarketChangePercent"]*100)}</div>', unsafe_allow_html=True)
    ...
    with c_ema:
        ...
        potential_levels = [
            ...
            (data['banker_price'], "💰 ทุนเจ้า (Big Lot 90 Days)") # ✅ ใส่กลับมาให้แล้ว!
        ]
        ...
        # ✅ Situation Insight (ตัวที่หายไป กู้คืนมาแล้ว!)
