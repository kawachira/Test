...
# --- 2. CSS ปรับแต่งความสวยงาม (คงเดิมตามที่คุณตั้งค่า) ---
st.markdown("""
    <style>
    /* ลดระยะห่างด้านบน */
    .block-container {
...
# --- 6. ฟังก์ชันสมอง AI (ใหม่: เพิ่มความหลากหลายและการสุ่ม) ---
def analyze_market_structure(price, ema20, ema50, ema200, rsi):
    ...
    # --- Scenario 1: ขาขึ้นแข็งแกร่ง (Strong Uptrend) ---
        ...
        with result_placeholder.container():
            ...
            if df is not None and not df.empty and len(df) > 200:
                ...
                with c1:
                    ...
                    # คำนวณ % ราคาปัจจุบันแบบ Manual (ตามเดิม)
                    ...
                    # คำนวณ % Pre/Post แบบ Manual (ตามเดิม)
                    pre_pct = 0.0
                    if pre_price and reg_price and reg_price != 0:
                        pre_pct = ((pre_price - reg_price) / reg_price) * 100
                        
                    post_pct = 0.0
                    if post_price and reg_price and reg_price != 0:
                        post_pct = ((post_price - reg_price) / reg_price) * 100
                    ...
                    if extra_market_html:
                        st.markdown(f"""
                        <div style="font-size:14px; color:#6b7280; display:flex; gap: 15px; flex-wrap: wrap; margin-top: 5px;">
                            {extra_market_html}
                        </div>
                        """, unsafe_allow_html=True)
                ...
                # แสดง Status Banner แบบใหม่ (ใช้ report)
