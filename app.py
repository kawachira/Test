...
import random  # <--- คง import random ไว้ตามเดิม
...
def analyze_market_structure(price, ema20, ema50, ema200, rsi):
    ...
    # --- Scenario 4: ช่วงฟื้นตัว / ไซด์เวย์ (Recovery / Sideway) ---
        ...
        # --- (แก้ไขใหม่) Logic หาแนวรับ/ต้าน ที่ถูกต้องเทียบกับราคาปัจจุบัน ---
        all_lines = [ema20, ema50, ema200]
        
        # หาแนวรับ: คือเส้นที่อยู่ "ต่ำกว่า" ราคา (เอาค่าที่ใกล้ราคาที่สุด = max)
        supports = [x for x in all_lines if x < price]
        nearest_sup = max(supports) if supports else price * 0.95
        
        # หาแนวต้าน: คือเส้นที่อยู่ "สูงกว่า" ราคา (เอาค่าที่ใกล้ราคาที่สุด = min)
        resistances = [x for x in all_lines if x > price]
        nearest_res = min(resistances) if resistances else price * 1.05

        action_2 = f"ผู้เล่นระยะสั้น: เล่นรอบในกรอบ แนวรับ {nearest_sup:.2f} แนวต้าน {nearest_res:.2f}"
